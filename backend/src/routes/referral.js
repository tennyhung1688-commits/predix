...
// ========== 导出供 trading.js 记录返佣 ==========
// feeRate 是 PrediX 收费率，用于反推 Polymarket 成本计算净利润
async function recordReferralEarnings(userId, tradeId, spreadFee, feeRate) {
  if (spreadFee <= 0) return;

  // 计算平台净利润
  // 净利润 = spreadFee - Polymarket 成本
  // Polymarket 成本 = spreadFee × (polyCostRate / feeRate)
  const polyCostRate = config.getPolyCostByFeeRate(feeRate);
  const netProfit = feeRate > 0 ? spreadFee * (1 - polyCostRate / feeRate) : spreadFee;

  if (netProfit <= 0) return; // 不亏不赚或亏损，不发返佣
  ...
  const l1Commission = netProfit * l1Tier.l1Rate;
  ...
  const l2Commission = netProfit * l2Tier.l2Rate;
  await prisma.referralEarning.create({
    data: {
